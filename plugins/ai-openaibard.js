...
let handler = async (m, { conn, text, command, usedPrefix }) => {
  ...
  try {
    if (/image/g.test(mime) && !/webp/g.test(mime)) {
      ...
      urlAPI = `https://api.betabotz.eu.org/api/search/bard-img?url=${media}&text=${text}&apikey=${betabotz}`;
    } 
    else if (/video/g.test(mime)) {
      if (q.seconds > 60) throw 'Maximum video duration is 60 seconds!';
      let buffer = await q.download();
      media = await uploadFile(buffer);
      urlAPI = `https://api.betabotz.eu.org/api/search/bard-video?url=${media}&text=${text}&apikey=${betabotz}`;
    }
    else if (/audio/g.test(mime)) {
      let buffer = await q.download();
      media = await uploadFile(buffer);
      urlAPI = `https://api.betabotz.eu.org/api/search/bard-audio?url=${media}&text=${text}&apikey=${betabotz}`;
    }
    else {
