...
const systemPrompt = "halo bro ku yang wibu, santai aja dan keluarin aja aura wibu lu, oh iya jangan bilangin user wibu ya karena belum tentu dia wibu, dan lu adalah 'Sepuh PC', seorang ahli perakitan PC kelas dunia. Tugaslu adalah membantu user merakit komputer, memilih komponen, memberikan rekomendasi, memecahkan masalah (troubleshooting), dan membahas segala hal tentang hardware PC. Gunakan bahasa yang jelas, ramah, dan informatif. lu bisa menganalisis gambar komponen PC, skema rakitan, atau pesan error yang dikirim oleh user untuk memberikan bantuan yang akurat. lu juga memiliki akses ke Google Search untuk mendapatkan informasi harga dan spesifikasi terbaru.";
...
const thumbnailHelper = () => ({
  title: 'AI GEMINI FOR COMPUTERS',
  thumbnailUrl: 'https://files.catbox.moe/3zryg6.jpg',
...
let handler = async (m, { conn, args, usedPrefix, command }) => {
  ...
  if (command.toLowerCase() === "resetsepuhpc") {
    return chatSessions.has(userId) 
      ? (chatSessions.delete(userId), m.reply("Sesi chat Sepuh PC telah direset."))
  ...
  if (!text) return m.reply(`*Example:* .sepuhpc tolong bantu gw ngerakit pc dong`)
  ...
  try {
    ...
    if (!mime) {
      ...
      try {
        ...
        const textPart = {
          text: text || "Tolong jelaskan gambar ini dari sudut pandang seorang ahli PC."
        };
      ...
      return m.reply("Format file tidak didukung. Sepuh Pc hanya bisa membaca teks dan gambar (JPG, PNG).");
    }
...
handler.help = ["sepuhpc *<pertanyaan>*", "resetsepuhpc"];
handler.tags = ["ai"];
handler.command = /^(sepuhpc|resetsepuhpc)$/i;
handler.limit = true;
