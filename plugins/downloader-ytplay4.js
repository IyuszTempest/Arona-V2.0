...
let handler = async (m, { conn, args, usedPrefix, command }) => {
    ...
    try {
        ...
        const apiUrl = `https://api.lolhuman.xyz/api/ytplay2?apikey=${global.lolkey}&query=${encodeURIComponent(query)}`;
        ...
        let message = `🎵 *YouTube Play 2* 🎵\n\n`;
        message += `*Judul:* ${song.title}\n`;
        message += `*Artis:* ${song.author}\n`;
...
handler.help = ['ytplay4 <query>'];
handler.tags = ['downloader'];
handler.command = /^(ytplay4)$/i;
handler.limit = true;
