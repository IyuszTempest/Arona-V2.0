...
const defaultMenu = {
    ...
    Berikut adalah daftar command untuk para wibu dan pecinta anime:`,
    header: '┌───「 *%category* 」',
...
let handler = async (m, { conn, usedPrefix: _p }) => {
    try {
        ...
        menuCategoryContent += defaultMenu.header.replace(/%category/g, 'MENU WIBU') + '\n';
        
        // Filter command khusus untuk 'anime'
        let categoryCommands = help.filter(menu => menu.tags && menu.tags.includes('anime') && menu.help && menu.help[0]);
        ...
        menuCategoryContent += '\n' + defaultMenu.after.replace(/<category>/g, 'Menu Wibu');
        ...
        let imageUrl = "./arona/menuanime.jpg"; // Bisa diganti gambar anime kalo ada
        ...
        conn.reply(m.chat, 'Waduh, menu wibu lagi error nih, gomenasai >.<', m);
        console.error("Error di menuanime.js:", e);
    }
}

handler.help = ['menuanime', 'menuwibu'];
handler.tags = ['main'];
handler.command = /^(menuanime|menuwibu)$/i
handler.exp = 3
