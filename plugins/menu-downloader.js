...
const defaultMenu = {
    ...
    Berikut adalah daftar command untuk kategori Downloader:`,
    header: '┌───「 *%category* 」',
...
let handler = async (m, { conn, usedPrefix: _p }) => {
    try {
        ...
        menuCategoryContent += defaultMenu.header.replace(/%category/g, 'MENU DOWNLOADER') + '\n';
        
        // Filter command khusus untuk 'downloader'
        let categoryCommands = help.filter(menu => menu.tags && menu.tags.includes('downloader') && menu.help && menu.help[0]);
        ...
        menuCategoryContent += '\n' + defaultMenu.after.replace(/<category>/g, 'Menu Downloader');
        ...
        await conn.sendMessage(m.chat, {
            ...
            contextInfo: {
                mentionedJid: [m.sender],
                 externalAdReply: {
                        title: `Menu Kategori: MENU DOWNLOADER`,
                        body: global.namebot,
                        mediaType: 1,
                        previewType: 0,
        ...
        conn.reply(m.chat, 'Waduh, menu downloader lagi error nih. Coba lagi nanti ya.', m);
        console.error("Error di menudownloader.js:", e);
    }
}
