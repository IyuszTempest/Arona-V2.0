...
let handler = async (m, { text, usedPrefix, command }) => {
    if (!text) throw `*Example:* ${usedPrefix + command} IyuszTempest`   
    try {     
        let json = await fetch(`https://api.betabotz.eu.org/api/stalk/github?username=${text}&apikey=${betabotz}`).then(res => res.json());
...
handler.help = ['ghstalk2 <username>']
handler.tags = ['stalk']
handler.command = /^(ghstalk2|githubstalk2)$/i
handler.limit = true
