...
let handler = async (m, {
  ...
  if (command == 'ffstalk') {
    if (!text) throw `Example : ${usedPrefix + command} 2153113100`;
    m.reply(wait);
    try {
      let ff = await fetch(`https://api.betabotz.eu.org/api/stalk/ff?id=${text}&apikey=${betabotz}`).then(res => res.json());
  ...
  if (command == 'mlstalk') {
    ...
    try {
      let ml = await fetch(`https://api.betabotz.eu.org/api/stalk/ml?id=${id}&server=${server}&apikey=${betabotz}`).then(res => res.json());
  ...
  if (command == 'mlstalk2') {
    ...
    let [id, server] = text.split('|');
    try {
      let ml = await fetch(`https://api.betabotz.eu.org/api/stalk/ml-v2?id=${id}&server=${server}&apikey=${betabotz}`).then(res => res.json());
  ...
  if (command == 'supersusstalk') {
    ...
    m.reply(wait);
    try {
      let sus = await fetch(`https://api.betabotz.eu.org/api/stalk/supersus?id=${text}&apikey=${betabotz}`).then(res => res.json());
  ...
  if (command == 'npmstalk') {
    ...
    m.reply(wait);
    try {
      let npm = await fetch(`https://api.betabotz.eu.org/api/stalk/npm?name=${text}&apikey=${betabotz}`).then(res => res.json());
  ...
  if (command == 'repostalk') {
    ...
    m.reply(wait);
    try {
      let repo = await fetch(`https://api.betabotz.eu.org/api/stalk/repo?repo=${text}&apikey=${betabotz}`).then(res => res.json());
  ...
  if (command == 'genshinstalk' || command == 'stalkgenshin' || command == 'gistalk') {
    ...
    m.reply(wait);
    try {
      let genshin = await fetch(`https://api.betabotz.eu.org/api/stalk/genshin?id=${text}&apikey=${betabotz}`).then(res => res.json());
  ...
  if (command == 'hokstalk') {
    ...
    m.reply(wait);
    try {
      let { result } = await fetch(`https://api.betabotz.eu.org/api/stalk/hok?id=${text}&apikey=${betabotz}`).then(res => res.json());
...
handler.command = handler.help = ['ffstalk2', 'mlstalk2', 'mlstalk3', 'supersusstalk2', 'npmstalk2', 'repostalk2', 'genshinstalk', 'gistalk', 'hokstalk'];
handler.tags = ['stalk'];
handler.limit = true;
